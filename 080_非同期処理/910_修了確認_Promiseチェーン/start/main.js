...
// 	console.log(`--${bob.name}'s timeline--`);
// 	const friends = await myFetch(`friendsOf${bob.id}.json`);
// 	for (const id of friends.friendIds) {
// 		const friend = await myFetch(`user${id}.json`);
// 		const message = await myFetch(`message${friend.latestMsgId}.json`);
...
(async function () {
  ...
  const friendList = await myFetch(`friendsOf${bob.id}.json`);
